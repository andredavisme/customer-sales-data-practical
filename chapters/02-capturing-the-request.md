...
| II | `req_owner` | Request Owner | Internal user or team responsible for the request |
...
SELECT
  req_owner,
  COUNT(req_id) AS stalled_requests,
  ROUND(AVG(CURRENT_DATE - req_date::DATE), 1) AS avg_days_in_review
FROM request
WHERE req_status = 'In Review'
GROUP BY req_owner
...
2. Look at the last ten requests your team processed. Could you reconstruct when each one arrived, how it arrived, and who was responsible for it? If not, which field — date, channel, or owner — would have made the biggest difference?
